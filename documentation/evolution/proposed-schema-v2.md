...
│                      SCHEMA V2                              │
├─────────────────────────────────────────────────────────────┤
│  MODEL        │  What data exists (fields, variables)       │
├───────────────┼─────────────────────────────────────────────┤
│  UI           │  How it looks (layout, theme, components)   │
├───────────────┼─────────────────────────────────────────────┤
│  LOGIC        │  How it behaves (computed, validation, fx)  │
├───────────────┼─────────────────────────────────────────────┤
│  INTEGRATIONS │  External data (APIs, master data)          │
├───────────────┼─────────────────────────────────────────────┤
│  ACCESS       │  Who can do what (roles, permissions)       │
└─────────────────────────────────────────────────────────────┘
```
...
NumberField    → min, max, integer, decimals  
SelectField    → options, multi, searchable
FileField      → accept, maxSize, capture
LocationField  → geoTagging, manualPick, accuracy
```
...
- Conditional visibility → `logic.visibility`
- Role-based access → `access.rules`
...
logic:
  cascades:
    locationCascade:
      ...
      cascadeDownstream: true  # state cleared → clear district, block, village
...
- 36 States → 700+ Districts → 6000+ Blocks → 90,000+ Villages
...
integrations:
  masters:
    ...
    locations:
      ...
      # Result: 90k rows → ~2.5k per state, loaded on demand
...
|----------|-------------|---------|
| `eager` | Small datasets (<1000 rows), always needed | Countries, Categories |
| `lazy` | Medium datasets, might not be needed | Products, Vendors |
| `onDemand` | Large datasets, hierarchical | Locations (state→district→village) |
...
→ Look at `model.fields`

### "I want to see how it looks"
→ Look at `ui.pages`, `ui.layout`, `ui.fields`

### "I want to understand the business logic"
→ Look at `logic.computed`, `logic.validations`, `logic.effects`

### "I want to see what external data it uses"
→ Look at `integrations.apis`, `integrations.masters`

### "I want to know who can do what"
→ Look at `access.roles`, `access.rules`
...
│  UI LAYER                                                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐    │
│  │   Pages     │  │   Layout    │  │   Theme     │  │ Field UI    │    │
│  │  (wizard)   │  │  (grid)     │  │  (colors)   │  │ (labels)    │    │
│  └─────────────┘  └─────────────┘  └─────────────┘  └─────────────┘    │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┼───────────────┐
                    ▼               ▼               ▼
┌──────────────────────┐  ┌──────────────────┐  ┌──────────────────────┐
│    MODEL LAYER       │  │   LOGIC LAYER    │  │   ACCESS LAYER       │
│  ┌────────────────┐  │  │  ┌────────────┐  │  │  ┌────────────────┐  │
│  │ Fields (data)  │◄─┼──┼─►│ Computed   │  │  │  │ Roles          │  │
│  ├────────────────┤  │  │  ├────────────┤  │  │  ├────────────────┤  │
...
│  INTEGRATIONS LAYER                                                      │
│  ┌─────────────────────────────┐  ┌─────────────────────────────────┐  │
│  │ APIs (external services)    │  │ Masters (reference data)        │  │
...
```
            ...
            ▼                   ▼                   ▼
    ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
    │ Data/Storage? │   │ Presentation? │   │   Behavior?   │
    └───────┬───────┘   └───────┬───────┘   └───────┬───────┘
            │                   │                   │
            ▼                   ▼                   ▼
    ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
    │    MODEL      │   │      UI       │   │    LOGIC      │
    │               │   │               │   │               │
    │ • field types │   │ • labels      │   │ • formulas    │
    │ • validation  │   │ • placeholders│   │ • validations │
    │   constraints │   │ • layout      │   │ • effects     │
    │ • variables   │   │ • theme       │   │ • visibility  │
    └───────────────┘   └───────────────┘   └───────────────┘
